...
Status Push(Stack& S, SElemType e) {
	...
	if (S.top - S.base >= S.staticsize)//栈满则重新分配空间
	{
		...
		S.top = (S.base + S.staticsize);//使得S.top重新指向原栈顶，因为realloc
		S.staticsize += STACK_INIT_SIZE;
	}
	*S.top++ = e;//top指向待插入位置
	return OK;
...
Status StackTraverse(Stack S, Status(*visit)(SElemType)) {
	//从栈底元素到栈顶元素依次执行visit函数，用于输出栈中元素
	SElemType* p = S.base;
...
Status MakeMaze(MazeType& maze) {//生成迷宫，0表示PATH，1表示WALL
	PosType m;
	srand(time(NULL));
	for (m.y = 0; m.y <= N + 1; m.y++) { maze[0][m.y] = WALL; maze[M + 1][m.y] = WALL; }//表示上下的墙
	for (m.x = 1; m.x <= M; m.x++) { maze[m.x][0] = WALL; maze[m.x][N+1] = WALL; }//表示左右的墙
	for (m.x = 1; m.x <= M; m.x++)
		for (m.y = 1; m.y <= N; m.y++)
			maze[m.x][m.y] = rand() % 2;                                        //随机生成迷宫
	return OK;
}
...
Status PassMaze(MazeType& maze, PosType start, PosType end, Stack& S) {
	...
	int curstep = 1;
	curpos = start;
	if (maze[curpos.x][curpos.y] != PATH) {             //判断入口是否通
		printf_s("当前迷宫没有入口\n");
		return FALSE;
	}
	do {
		if (maze[curpos.x][curpos.y] == PATH) {            //当前位置是通道
			e.ord = curstep;
			e.seat = curpos;
			e.di = 1;
			Push(S, e);                                    //压栈
			...
			while (!StackEmpty(S) && e.di == 4) {
				maze[e.seat.x][e.seat.y] = BACK;       //撞墙回头
				Pop(S, e);  //出栈
				curstep--;
			...
			if (e.di < 4) {                           //栈顶位置有其他方向可以选择
				Pop(S, e);
				e.di++;
				Push(S, e); maze[e.seat.x][e.seat.y] = -e.di;//注意前进方向踪迹与e.di互为相反数，因为是向右转
