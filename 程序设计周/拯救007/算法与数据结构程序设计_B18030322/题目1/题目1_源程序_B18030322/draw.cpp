...
int graph_draw()
{
	...
	for (int i = 1; i <= 25; i++)
	{
		DWORD a[4] = { 5, 15, 3, 8 };                                                                          //设置线的类型
		setlinestyle(PS_USERSTYLE | PS_ENDCAP_FLAT, 1, a, 4);
		line((width / 2 - 250), (height / 2 - 250) + 20 * i, (width / 2 + 250), (height / 2 - 250) + 20 * i);  //横线
		line((width / 2 - 250) + 20 * i, (height / 2 - 250), (width / 2 - 250) + 20 * i, (height / 2 + 250));  //竖线
	}
	//文字显示
	...
	while (true)
	{
		...
		case WM_LBUTTONDOWN:
			...
			if (m.mkCtrl)
			{
				if (m.x >= width / 2 + 250 || m.x <= width / 2 - 250 || m.y <= height / 2 - 250 || m.y >= height / 2 + 250)  //限制只能在湖面内画点
					break;
				// 画一个大方块
				else
...
Graph read_case(FILE* InFile, int num, Vertex* Bank, Deque D)
{
	...
	*Bank = 0;
	...
	if (CheckForEnd(0, 0, JamesJump + ISLAND_DIAMETER / 2.0))                  //如果能一步跳出
	{
		for (i = 0; i < (num << 1); i++)                                       //此循环是为了保证屏蔽鳄鱼位置，否则输出鳄鱼个数多个-1
			fscanf_s(InFile, "%d", &x);
		*Bank = 1;                                                             //步数为1
	}
	else if (num > 0)    //007必须经过鳄鱼头上的情况
	{
		num += 2;
		G = GraphNew(num);
		for (i = 2; i < num; i++)   //第三个node开始是鳄鱼（前两个分别是鳄鱼个数和步长）
		{
			fscanf_s(InFile, "%d", &x);  //读取第i个鳄鱼的xy坐标
			fscanf_s(InFile, "%d", &y);
			G[i].X = x;
			G[i].Y = y;
			if (CheckForStart(x, y, JamesJump))  //判断是否能跳上该点，如果能跳上去
			{
				G[i].Path = 1;  //007可以跳到
				G[i].Step = 1;  //一步
				if (CheckForEnd(x, y, JamesJump))//判断是否能跳出，如果能从这个点跳出去
				{
					*Bank = i;//007可以跳出
					Times = (num - i - 1) << 1;
		...
		while (!IsEmpty(D))  //只经过一个鳄鱼无法跳出，必须还要跳到其他鳄鱼的情况
		{
			...
			{
				if ((G[i].Step > G[V].Step + 1) && CheckForConnect(G, V, i, JamesJump))//如果能从当前鳄鱼跳到其他鳄鱼
				{
					G[i].Path = V;
					G[i].Step = G[V].Step + 1;//步数加一
					if ((G[i].Step < G[*Bank].Step) && CheckForEnd(G[i].X, G[i].Y, JamesJump))//如果能从当前鳄鱼跳到岸边就结束
						*Bank = i;
					else
						Inject(i, D);//不能跳出就将鳄鱼放入队列
				}
			}
		}
	}
